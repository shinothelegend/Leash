...
This project was built autonomously by an AI Coding Agent based on a single, comprehensive one-shot prompt.
...
- **The Graph Subgraph**: Definition of `schema.graphql` and the assemblyscript event handlers in `mapping.ts`, as well as fixing complex Graph CLI type-generation errors.
- **Next.js Frontend**: A complete, highly-polished web dashboard using Vanilla CSS with dark mode, animations, and premium styling, created iteratively without user feedback.
